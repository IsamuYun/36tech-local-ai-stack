...
function AiToolsChatbot() {
  ...
  const handleSubmit = async (event) => {
    ...
    setIsLoading(true);
    setMessage({ state: 'loading', text: 'Gemini is generating a response…' });

    try {
      const response = await fetch('/api/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ question: trimmedQuestion, language: 'en' }),
      });
      ...
      setMessage({ state: 'success', text: 'Response generated by Gemini.' });
      setQuestion('');
    } catch (error) {
      setMessage({
        state: 'error',
        text: error instanceof Error ? error.message : 'Unable to generate a response.',
      });
    } finally {
      setIsLoading(false);
    }
